...
*   **Token Ring con Épocas (Term Epochs)**: El acceso exclusivo al recurso crítico ([RecursoCritico.java](app/src/main/java/org/example/distributed/RecursoCritico.java)) se coordina haciendo circular un token lógico en anillo (`1 ➔ 2 ➔ 3 ➔ 1`) gestionado por [TokenRing.java](app/src/main/java/org/example/distributed/TokenRing.java).
    ...
    *   *Preservación de Épocas en P2P*: La transmisión y clonación de mensajes de peers conservan explícitamente el valor de época del coordinador, evitando que el token legítimo sea erróneamente descartado por inconsistencias en la época del clúster.
*   **Sincronización en Caliente (State Transfer)**: Cuando un servidor que estuvo caído se levanta de nuevo o se reincorpora al clúster, envía un mensaje `SYNC_REQUEST` a un peer activo. El peer responde con un `SYNC_RESPONSE` con todas las claves y definiciones de comandos aprendidos de [RecursoCritico.java](app/src/main/java/org/example/distributed/RecursoCritico.java), logrando consistencia eventual instantánea.
*   **Sintaxis Global de Aprendizaje y Visibilidad**: Si un usuario intenta usar la función `/aprender` con argumentos incorrectos (ej. `/aprender hola`), el servidor detecta el error sintáctico y difunde un mensaje de advertencia (`WARNING`) a todos los clientes del chat global. Adicionalmente, las solicitudes válidas de `/aprender` e invocación de comandos dinámicos aprendidos se difunden globalmente antes de procesarse, garantizando la visibilidad completa del historial del chat por todos los clientes.
...
*   **Bypass de Historial para el Bot**: Al reconectarse, el servidor omite enviarle la cola del historial acumulado del chat a `BotCentral`, evitando respuestas redundantes e invocaciones duplicadas de comandos antiguos.
...
*   **Deduplicación de Respuestas de Comandos**: `NodoBot` está diseñado para delegar las respuestas de comandos aprendidos almacenados en el recurso crítico de vuelta al servidor (retornando `null` al identificarlos localmente), evitando que tanto el bot como el servidor respondan duplicado en el chat.
